...
module DiscardPerformant
  ...
  module Model
    ...
    included do
      class_attribute :discard_datetime_column, :discard_flag_column
      self.discard_datetime_column = :discarded_at
      self.discard_flag_column = :discarded

      scope :kept, ->{ undiscarded }
      scope :undiscarded, ->{ where(discard_flag_column => true) }
      scope :discarded, ->{ where.not(discard_flag_column => false) }
      scope :with_discarded, ->{ unscope(where: discard_flag_column) }
    ...
    def discarded?
      self[self.class.discard_flag_column].present?
    end
    ...
    def discard
      ...
      run_callbacks(:discard) do
        update_columns(
          self.class.discard_datetime_column => Time.current,
          self.class.discard_flag_column => true,
        )
      end
    end
    ...
    def undiscard
      ...
      run_callbacks(:undiscard) do
        update_columns(
          self.class.discard_datetime_column => nil,
          self.class.discard_flag_column => false,
        )
      end
    end
